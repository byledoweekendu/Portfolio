...
export default function TodayTasks(props) {
   ...
   useEffect(() => {
    setReminders(allReminders.filter((item) => {
      if(item.name.includes(phrase) && option === 'Cykliczne' 
      && item.isCompleted === 'Nie' && (item.cyclicTask === "Codziennie" || item.cyclicTask === "Co tydzień" || item.cyclicTask === "Co miesiąc")){
        return allReminders;
      }
      }))
  }, []);
